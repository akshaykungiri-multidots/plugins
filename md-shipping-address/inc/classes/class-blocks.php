...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_SHIPPING_ADDRESS_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_SHIPPING_ADDRESS_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-shipping-address/' ) ) {
			$string = str_replace( 'md-shipping-address/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-shipping-address',
					'title' => __( 'MD Shipping Address Block', 'md-shipping-address' ),
