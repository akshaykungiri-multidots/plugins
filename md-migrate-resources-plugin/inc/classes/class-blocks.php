...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_MIGRATE_RESOURCES_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_MIGRATE_RESOURCES_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-migrate-resources/' ) ) {
			$string = str_replace( 'md-migrate-resources/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-migrate-resources',
					'title' => __( 'MD Migrate Resources Block', 'md-migrate-resources' ),
