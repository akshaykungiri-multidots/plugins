...
class Front
{
    ...
    {
        $md_prime_option_name = get_option('md_prime_option_name');
        if (!empty($md_prime_option_name['md_mailchimp_api_key']) && !empty($md_prime_option_name['md_mailchimp_prefix'])) {
            $md_mailchimp_email_field = $md_prime_option_name['md_mailchimp_email_field'];
            $submission = \WPCF7_Submission::get_instance();
            $posted_data = $submission->get_posted_data();
            if (isset($posted_data[$md_mailchimp_email_field]) && isset($posted_data['cf7_custom_md_mailchimp_list']) && !empty($posted_data['cf7_custom_md_mailchimp_list'])) {

                $list_id = sanitize_text_field($posted_data['cf7_custom_md_mailchimp_list']);
                $email_address = (isset($posted_data[$md_mailchimp_email_field]) ? sanitize_email($posted_data[$md_mailchimp_email_field]) : "");
                $client = new \MailchimpMarketing\ApiClient();
                $client->setConfig(
                    [
                        'apiKey' => $md_prime_option_name['md_mailchimp_api_key'],
                        'server' => $md_prime_option_name['md_mailchimp_prefix'],
                    ]
                );
                try {
                    //code...
