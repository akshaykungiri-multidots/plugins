<?php
/**
 * Registers the md-efi/md-posts block.
 *
...
/**
 *  Class for the md-efi/md-posts block.
 */
class MD_Posts extends Block_Base {
	...
	protected function setup_hooks() {
		...
		// Ajax hooks.
		add_action( 'wp_ajax_md_efi_load_more', array( $this, 'md_efi_load_more' ) );
		add_action( 'wp_ajax_nopriv_md_efi_load_more', array( $this, 'md_efi_load_more' ) );
	}
					...
					<?php if ( $query->max_num_pages > 1 ) : ?>
						<div class="md-posts__loadmore">
							<a href="javascript:void(0);" class="md-posts__loadmore-button btn-main" data-page="1" data-post-type="<?php echo esc_attr( $postType ); ?>" data-posts-per-page="<?php echo esc_attr( $postsToShow ); ?>" data-post-in-row="<?php echo esc_attr( $postInRow ); ?>">
								<?php esc_html_e( 'Load More', 'md-efi' ); ?>
							</a>
						</div>
					<?php endif; ?>
				<?php else : ?>
					<p><?php esc_html_e( 'No posts found.', 'md-efi' ); ?></p>
				<?php endif; ?>
			</div>
		</div>
		<?php
	 ...
	 * @since 1.0.0
	 */
	public function md_efi_load_more() {
		// Check nonce.
