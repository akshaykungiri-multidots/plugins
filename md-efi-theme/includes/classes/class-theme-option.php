...
class Theme_Option {
	...
	public function add_option_menu() {
		add_menu_page( 'Theme Options', 'Theme Options', 'manage_options', 'md-efi', array( $this, 'option_form' ), '', 50 );
	}
	...
	public function option_form() {
		?>
		<div class="wrap">
			<div class="md-efi-theme-options">
				<div class="md-efi-theme-options-header">
					<h1>MD Efi Theme Options</h1>
					<?php
					...
					<h2 class="nav-tab-wrapper">
						<a href="?page=md-efi&tab=general_options" class="nav-tab <?php echo esc_attr( 'general_options' === $active_tab ? 'nav-tab-active' : '' ); ?>"><?php echo esc_html__( 'General Settings', 'md-efi' ); ?></a>
					</h2>
					<span><?php settings_errors(); ?></span>
				</div>
				<form action='options.php' enctype="multipart/form-data" method='post'> 
					<?php
					settings_fields( 'md-efi-setting' );
					do_settings_sections( 'md-efi' );
					submit_button();
					?>
				</form>
			</div>
	...
	public function option_settings_init() {
		register_setting( 'md-efi-setting', 'md_efi_settings' );
		add_settings_section( 'md-efi-section', __( 'General Settings', 'md-efi' ), '__return_empty_string', 'md-efi' );
		add_settings_field(
			'md_efi_logo',
			__( 'Site Logo:', 'md-efi' ),
			...
			array(
				'type'  => 'file',
				'name'  => 'md_efi_settings[md_efi_img]',
				'value' => 'md_efi_img',
			)
		);
		add_settings_field(
			'md_efi_favicon',
			__( 'Site Favicon:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'  => 'file',
				'name'  => 'md_efi_settings[md_efi_favicon]',
				'value' => 'md_efi_favicon',
			)
		);
		add_settings_field(
			'md_efi_tagline',
			__( 'Site Tagline:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'  => 'text',
				'name'  => 'md_efi_settings[md_efi_tagline]',
				'value' => 'md_efi_tagline',
			)
		);
		add_settings_field(
			'md_efi_google_analytic',
			__( 'Google Analytic:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'  => 'textarea',
				'name'  => 'md_efi_settings[md_efi_google_analytic]',
				'value' => 'md_efi_google_analytic',
			)
		);
		add_settings_field(
			'md_efi_css_code',
			__( 'Additional CSS:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'  => 'textarea',
				'name'  => 'md_efi_settings[md_efi_css_code]',
				'value' => 'md_efi_css_code',
			)
		);
		add_settings_field(
			'md_efi_html_code',
			__( 'Additional html:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'  => 'textarea',
				'name'  => 'md_efi_settings[md_efi_html_code]',
				'value' => 'md_efi_html_code',
			)
		);
		add_settings_field(
			'md_efi_color_picker',
			...
			array(
				'type'  => 'color-picker',
				'name'  => 'md_efi_settings[md_efi_color_picker]',
				'value' => 'md_efi_color_picker',
			)
		);
		add_settings_field(
			'md_efi_wysiwyg',
			__( 'Text Editor:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'  => 'wysiwyg',
				'name'  => 'md_efi_settings[md_efi_wysiwyg]',
				'value' => 'md_efi_wysiwyg',
			)
		);
		add_settings_field(
			'md_efi_password',
			__( 'Manage Password:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'  => 'password',
				'name'  => 'md_efi_settings[md_efi_password]',
				'id'    => 'password',
				'value' => 'md_efi_password',
			)
		);
		add_settings_field(
			'md_efi_selectbox',
			__( 'Select Option:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'    => 'select',
				'name'    => 'md_efi_settings[md_efi_selectbox]',
				'value'   => 'md_efi_selectbox',
				'options' => array(
					'0' => '0',
					'1' => '1',
					'2' => '2',
		...
		add_settings_field(
			'md_efi_multiselect',
			__( 'Select Multiple Option:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'    => 'multicheck',
				'name'    => 'md_efi_settings[md_efi_multiselect]',
				'id'      => 'multicheck',
				'value'   => 'md_efi_multiselect',
				'options' => array(
					'0' => 'pizza',
					'1' => 'biryani',
					'2' => 'burger',
		...
		add_settings_field(
			'md_efi_radio',
			__( 'Select Radio Option:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'    => 'radio',
				'name'    => 'md_efi_settings[md_efi_radio]',
				'value'   => 'md_efi_radio',
				'options' => array(
					'0' => 'HTML',
					'1' => 'CSS',
		...
		add_settings_field(
			'md_efi_checkbox',
			__( 'Select Checkbox Option:', 'md-efi' ),
			array( $this, 'add_field' ),
			'md-efi',
			'md-efi-section',
			array(
				'type'  => 'checkbox',
				'id'    => 'checkbox',
				'name'  => 'md_efi_settings[md_efi_checkbox]',
				'value' => 'md_efi_checkbox',
		...
		add_settings_field(
			...
			array(
				'type'  => 'number',
				'name'  => 'md_efi_settings[md_efi_number]',
				'value' => 'md_efi_number',
			)
		);
	}
	...
	public function add_field( array $args ) {
		$options = get_option( 'md_efi_settings' );
		switch ( $args['type'] ) {
			case 'text':
	...
	public function file_callback( $args, $options ) {
		...
		<img class="md_efi_img" 
			...
			?>
		/>
		<input class="md_efi_img_url" 
			type="hidden" 
			name="<?php echo esc_attr( $args['name'] ); ?>" 
			size="60" 
			value="<?php echo isset( $options[ $args['value'] ] ) ? esc_attr( $options[ $args['value'] ] ) : ''; ?>"
		>
		<a href="#" class="md_efi_img_upload"><button>Upload</button></a>
		<a href="#" class="md_efi_img_remove"><button>Remove</button></a>
		<?php
	}
	...
	public function theme_option_assets() {
		...
		// Register styles.
		wp_register_style( 'theme-option-css', MD_EFI_BUILD_URI . '/themeoption.css', array(), filemtime( MD_EFI_BUILD_PATH . '/themeoption.css' ), 'all' );
		...
		wp_register_script( 'theme-option-js', MD_EFI_BUILD_URI . '/themeoption.js', array( 'jquery-core', 'wp-color-picker' ), filemtime( MD_EFI_BUILD_PATH . '/themeoption.js' ), true );
