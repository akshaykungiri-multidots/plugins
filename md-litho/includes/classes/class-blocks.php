...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'md_litho_custom_block_category' ) );
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( MD_LITHO_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_LITHO_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function md_litho_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-litho',
					'title' => __( 'MD Litho', 'md-litho' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-litho/' ) ) {
			$string = str_replace( 'md-litho/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config(): void {
		$blocks_config = apply_filters( 'md_litho_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'md_litho_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
