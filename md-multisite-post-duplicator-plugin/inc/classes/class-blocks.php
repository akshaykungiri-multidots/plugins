 ...
 * Dynamic Blocks.
 *
 * @package md-multisite-post-duplicator
 */
...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_MULTISITE_POST_DUPLICATOR_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_MULTISITE_POST_DUPLICATOR_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		$namespace = 'MD_Multisite_Post_Duplicator\Blocks\\';

		// Remove namespace from block name.
		if ( false !== strpos( $string, 'md-multisite-post-duplicator/' ) ) {
			$string = str_replace( 'md-multisite-post-duplicator/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-multisite-post-duplicator',
					'title' => __( 'MD Multisite Post Duplicator Block', 'md-multisite-post-duplicator' ),
