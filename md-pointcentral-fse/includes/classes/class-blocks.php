...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'md_pointcentral_fse_custom_block_category' ) );
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( MD_POINTCENTRAL_FSE_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_POINTCENTRAL_FSE_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function md_pointcentral_fse_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-pointcentral-fse',
					'title' => __( 'MD Pointcentral Fse', 'md-pointcentral-fse' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-pointcentral-fse/' ) ) {
			$string = str_replace( 'md-pointcentral-fse/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config(): void {
		$blocks_config = apply_filters( 'md_pointcentral_fse_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'md_pointcentral_fse_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
