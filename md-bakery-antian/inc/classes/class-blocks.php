...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_BAKERY_ANTIAN_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_BAKERY_ANTIAN_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-bakery-antian/' ) ) {
			$string = str_replace( 'md-bakery-antian/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-bakery-antian',
					'title' => __( 'MD Bakery Antian Block', 'md-bakery-antian' ),
