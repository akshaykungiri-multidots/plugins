...
class Assets {
	...
	protected function setup_hooks() {
		...
		add_action( 'enqueue_block_editor_assets', array( $this, 'md_enqueue_editor_assets' ) );
		/**
	...
	public function move_scripts_to_footer() {
		...
		wp_enqueue_script( 'jquery-core', '/wp-includes/js/jquery/jquery.min.js', array(), MD_ANITIAN_FSE_FULL_THEME_VERSION, true );
	}
	 ...
	 * @since 1.0.0
	 */
	public function md_enqueue_editor_assets() {
		
		// Editor Script.
		wp_enqueue_script(
			'md-anitian-fse-full-blocks-js',
			MD_ANITIAN_FSE_FULL_BUILD_URI . '/blocks.js',
			array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'wp-components', 'wp-data' ),
			filemtime( MD_ANITIAN_FSE_FULL_BUILD_PATH . '/blocks.js' ),
			true
		);
	...
	public function hook_critical_css() {
		...
		$response = wp_remote_get( MD_ANITIAN_FSE_FULL_BUILD_URI . '/inline.css' );   // load template output in buffer.

		if ( ! is_wp_error( $response ) ) {
			$css = wp_remote_retrieve_body( $response );
			wp_register_style( 'md-anitian-fse-full-inline-css', false, array(), true, true );
			wp_add_inline_style( 'md-anitian-fse-full-inline-css', $css );
			wp_enqueue_style( 'md-anitian-fse-full-inline-css' );
		}
	}
	...
	public function register_styles() {
		...
		// Register styles.
		wp_register_style( 'main-css', MD_ANITIAN_FSE_FULL_BUILD_URI . '/main.css', array(), filemtime( MD_ANITIAN_FSE_FULL_BUILD_PATH . '/main.css' ), 'all' );
	...
	public function register_scripts() {
		...
		// Register scripts.
		wp_register_script( 'main-js', MD_ANITIAN_FSE_FULL_BUILD_URI . '/main.js', array( 'jquery-core' ), filemtime( MD_ANITIAN_FSE_FULL_BUILD_PATH . '/main.js' ), true );
	...
	public function enqueue_editor_assets() {
		...
		if ( is_admin() ) {
			wp_enqueue_style(
				'md-anitian-fse-full-editor-js',
				MD_ANITIAN_FSE_FULL_BUILD_URI . '/geditor.css',
				array(),
				filemtime( MD_ANITIAN_FSE_FULL_BUILD_PATH . '/geditor.css' ),
				'all'
			);
		}
