...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_THREATDOWN_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_THREATDOWN_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-threatdown/' ) ) {
			$string = str_replace( 'md-threatdown/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-threatdown',
					'title' => __( 'MD Threatdown Block', 'md-threatdown' ),
