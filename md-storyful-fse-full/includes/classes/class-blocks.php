...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'md_storyful_fse_full_custom_block_category' ) );
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( MD_STORYFUL_FSE_FULL_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_STORYFUL_FSE_FULL_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function md_storyful_fse_full_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-storyful-fse-full',
					'title' => __( 'MD Storyful Fse Full', 'md-storyful-fse-full' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-storyful-fse-full/' ) ) {
			$string = str_replace( 'md-storyful-fse-full/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config(): void {
		$blocks_config = apply_filters( 'md_storyful_fse_full_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'md_storyful_fse_full_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
