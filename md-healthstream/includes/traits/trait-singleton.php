...
namespace MD_HEALTHSTREAM\Includes\Traits;
...
trait Singleton {
	...
	final public static function get_instance() {
		...
		if ( ! isset( static::$_instance[ $called_class ] ) ) {
			...
			/*
			 * Dependent items can use the `md_healthstream_singleton_init_{$called_class}` hook to execute code
			 * immediately after _init() is called.
			 */
			do_action( sprintf( 'md_healthstream_singleton_init_%s', $called_class ) );
	...
	/**
	 * Unset the instance of the class.
	 *
	 * This is useful for unit testing, or for any other reason
	 * where you need to reset the state of the class.
	 */
	public static function unset_instance() {
		$called_class = get_called_class();

		if ( ! isset( static::$_instance[ $called_class ] ) ) {
			unset( self::$_instance[ $called_class ] );
		}
	}
} // End trait
