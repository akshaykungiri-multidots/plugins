...
( async () => {
	...
	const { description, icon, name, slug, namespace, category } = await inquirer.prompt( [
		{
			type: 'input',
			name: 'namespace',
			message: 'What is the namespace prefix of the block?',
			// Default namespace.
			default() {
				return 'md-healthstream';
			},
			validate( input ) {
				// Only allow lowercase letters and hyphens.
				if ( ! /^[a-z\-]+$/.test( input ) ) {
					return 'Namespace can only contain hyphens and lowercase letters.';
				}
		...
		},
		{
			...
			validate( input ) {
				...
				if ( ! /^[a-z0-9\-]+$/.test( input ) ) {
					return 'Slug can only contain hyphens, numbers, and lowercase letters.';
				}
		...
		},
		{
			...
			validate( input ) {
				// Only allow lowercase letters and hyphens.
				if ( ! /^[a-z\-]+$/.test( input ) ) {
					return 'Category can only contain hyphens and lowercase letters.';
				}
	...
	// Return name or slugified name.
	const mUsePath = unslugify( namespace ).toUpperCase();
	const textDomain = unslugify( namespace ).toLowerCase();
	const blockCategory = unslugify( category ).toLowerCase();
	...
	const view = {
		...
		slug: modifiedSlug,
		textDomain: slugify( textDomain, '-' ),
		phpPath: slugify( mUsePath, '_' ),
		category: slugify( blockCategory, '-' ),
	};
