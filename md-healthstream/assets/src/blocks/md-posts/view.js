...
(function ($) {
  ...
  $(document).ready(function () {
      ...
      const postAttributes = $(".md-posts")
        .find('input[name="md-posts__loadmore-post_attributes"]')
        .val();
      const taxonomiesVal = [];
      $(".md-posts__filter .md-posts__filter-checkbox input[type='checkbox']:checked").each(function () {
        const termVal = $(this).val();
        if (termVal === "") {
          return;
        }
        const taxonomyVal = $(this).data("taxonomy");
        taxonomiesVal.push({ taxonomy: taxonomyVal, term: termVal });
      });
      const currentPage = $(".md-posts")
        .find('input[name="md-posts__loadmore-current_page"]')
        .val();
  
      const data = {
        action: "md_healthstream_load_more",
    ...
    $(document).on(
      "click",
      ".md-posts__filter .md-posts__filter-checkbox input[type='checkbox']",
      function () {
        $(".md-posts__grid").html("");
        $(".md-posts")
          .find('input[name="md-posts__loadmore-current_page"]')
