...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_CIRCLE_PLUS_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_CIRCLE_PLUS_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-circle-plus/' ) ) {
			$string = str_replace( 'md-circle-plus/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-circle-plus',
					'title' => __( 'MD Circle Plus Block', 'md-circle-plus' ),
