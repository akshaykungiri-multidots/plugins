...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_WOO_DISCOUNT_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_WOO_DISCOUNT_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-woo-discount/' ) ) {
			$string = str_replace( 'md-woo-discount/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-woo-discount',
					'title' => __( 'MD Woo Discount Block', 'md-woo-discount' ),
