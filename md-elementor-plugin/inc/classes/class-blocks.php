...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_ELEMENTOR_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_ELEMENTOR_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-elementor/' ) ) {
			$string = str_replace( 'md-elementor/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-elementor',
					'title' => __( 'MD Elementor Block', 'md-elementor' ),
