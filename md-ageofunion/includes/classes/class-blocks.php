...
class Blocks {
	...
	protected function setup_hooks(): void {
		...
		add_filter( 'block_categories_all', array( $this, 'md_ageofunion_custom_block_category' ) );
	...
	public function register_theme_blocks(): void {
		...
		if ( file_exists( MD_AGEOFUNION_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_AGEOFUNION_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function md_ageofunion_custom_block_category( array $categories ): array {
		return array_merge(
			array(
				array(
					'slug'  => 'md-ageofunion',
					'title' => __( 'MD Ageofunion', 'md-ageofunion' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-ageofunion/' ) ) {
			$string = str_replace( 'md-ageofunion/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config(): void {
		$blocks_config = apply_filters( 'md_ageofunion_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'md_ageofunion_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
