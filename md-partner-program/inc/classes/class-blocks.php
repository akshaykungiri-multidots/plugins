...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_PARTNER_PROGRAM_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_PARTNER_PROGRAM_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-partner-program/' ) ) {
			$string = str_replace( 'md-partner-program/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-partner-program',
					'title' => __( 'MD Partner Program Block', 'md-partner-program' ),
