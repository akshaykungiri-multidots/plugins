  ...
  $(document).ready(function () {
    function mdLoadPosts(currentPage = 1) {
      const postAttributes = $(".md-posts")
        .find('input[name="md-posts__loadmore-attributes"]')
        .val();
      const taxonomiesArr = [];
      $(".insights-filter-dropbox .category-filter-wrapper select").each(
        function () {
          const taxonomyVal = $(this).attr("name");
          const termVal = $(this).val();
          taxonomiesArr.push({ taxonomy: taxonomyVal, term: termVal });
        }
      );
      const searchVal = $(".products-list__search-container input").val();
      const data = {
        action: "md_woodward_load_more",
    ...
    $(document).on(
      "click",
      "ul.page-numbers li a.page-numbers:not(.current)",
      function (e) {
        e.preventDefault();
