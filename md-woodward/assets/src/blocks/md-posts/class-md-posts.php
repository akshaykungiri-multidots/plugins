...
/**
 * Registers the md-woodward/md-posts block.
 *
...
/**
 *  Class for the md-woodward/md-posts block.
 */
class MD_Posts extends Block_Base
{
	...
	{
		add_filter('md_woodward_gutenberg_blocks_config', array($this, 'localize_block_data'));

		// Ajax hooks.
		add_action('wp_ajax_md_woodward_load_more', array($this, 'md_woodward_load_more'));
		add_action('wp_ajax_nopriv_md_woodward_load_more', array($this, 'md_woodward_load_more'));
	}
			...
			<div class="md-posts insights-filter-main <?php echo esc_attr($postType); ?>">
				...
				<div class="products-list__search-container">
					<div class="products-list__search-text-wrapper">
						<input type="text" id="products-list__search-text" class="search-input products-list__search-input" placeholder="<?php esc_attr_e('Search', 'md-woodward'); ?>">
						<button class="clear-btn products-list__search-text-reset-button" style="display: none;" ><?php esc_html_e('Close', 'md-woodward'); ?></button>
					</div>
					<button class="search-btn products-list__search-button"><?php esc_html_e('Search', 'md-woodward'); ?></button>
				</div>
				<div class="insights-filter-dropbox-main">
					<div class="insights-filter-dropbox">
						...
						?>
						<button class="md-reset-filter products-list__reset-button"><?php esc_html_e('Reset Filter', 'md-woodward'); ?></button>
					</div>
				</div>
				<?php endif; ?>
				<div class="insights-results-main">
					...
					<?php else : ?>
						<p><?php esc_html_e('No posts found.', 'md-woodward'); ?></p>
					<?php endif; ?>
				</div>
			</div>
		</div>
<?php
	 ...
	 * @since 1.0.0
	 */
	public function md_woodward_load_more()
	{
		// Check nonce.
