...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_YOAST_MIGRATION_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_YOAST_MIGRATION_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-yoast-migration/' ) ) {
			$string = str_replace( 'md-yoast-migration/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-yoast-migration',
					'title' => __( 'MD Yoast Migration Block', 'md-yoast-migration' ),
