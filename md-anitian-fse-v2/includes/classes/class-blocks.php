...
class Blocks {
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( MD_ANITIAN_FSE_V2_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_ANITIAN_FSE_V2_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public function md_anitian_fse_full_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-anitian-fse-v2',
					'title' => __( 'MD Anitian Fse V2', 'md-anitian-fse-v2' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-anitian-fse-v2/' ) ) {
			$string = str_replace( 'md-anitian-fse-v2/', '', $string );
		}
