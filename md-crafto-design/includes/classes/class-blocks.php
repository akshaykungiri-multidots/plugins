...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'md_crafto_design_custom_block_category' ) );
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( MD_CRAFTO_DESIGN_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_CRAFTO_DESIGN_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function md_crafto_design_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-crafto-design',
					'title' => __( 'MD Crafto Design', 'md-crafto-design' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-crafto-design/' ) ) {
			$string = str_replace( 'md-crafto-design/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config(): void {
		$blocks_config = apply_filters( 'md_crafto_design_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'md_crafto_design_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
