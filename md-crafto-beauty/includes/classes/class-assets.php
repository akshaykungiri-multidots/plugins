...
class Assets {
	...
	public function move_scripts_to_footer() {
		...
		wp_enqueue_script( 'jquery-core', '/wp-includes/js/jquery/jquery.min.js', array(), MD_CRAFTO_BEAUTY_THEME_VERSION, true );
	}
	...
	public function hook_critical_css() {
		...
		$response = wp_remote_get( MD_CRAFTO_BEAUTY_BUILD_URI . '/inline.css' );   // load template output in buffer.

		if ( ! is_wp_error( $response ) ) {
			$css = wp_remote_retrieve_body( $response );
			wp_register_style( 'md-crafto-beauty-inline-css', false, array(), true, true );
			wp_add_inline_style( 'md-crafto-beauty-inline-css', $css );
			wp_enqueue_style( 'md-crafto-beauty-inline-css' );
		}
	}
	...
	public function register_styles() {
		...
		// Register styles.
		wp_register_style( 'main-css', MD_CRAFTO_BEAUTY_BUILD_URI . '/main.css', array(), filemtime( MD_CRAFTO_BEAUTY_BUILD_PATH . '/main.css' ), 'all' );
	...
	public function register_scripts() {
		...
		// Register scripts.
		wp_register_script( 'main-js', MD_CRAFTO_BEAUTY_BUILD_URI . '/main.js', array( 'jquery-core' ), filemtime( MD_CRAFTO_BEAUTY_BUILD_PATH . '/main.js' ), true );
	...
	public function enqueue_editor_assets() {
		...
		if ( is_admin() ) {
			wp_enqueue_style(
				'md-crafto-beauty-editor-css',
				MD_CRAFTO_BEAUTY_BUILD_URI . '/geditor.css',
				array(),
				filemtime( MD_CRAFTO_BEAUTY_BUILD_PATH . '/geditor.css' ),
				'all'
			);
			...
			wp_enqueue_script(
				...
				MD_CRAFTO_BEAUTY_BUILD_URI . '/blocks.js',
				array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'wp-components', 'wp-data', 'jquery', 'slick' ),
				filemtime( MD_CRAFTO_BEAUTY_BUILD_PATH . '/blocks.js' ),
				true
			);
		}
