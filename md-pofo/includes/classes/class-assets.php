...
class Assets {
	...
	public function move_scripts_to_footer(): void {
		...
		wp_enqueue_script( 'jquery-core', '/wp-includes/js/jquery/jquery.min.js', array(), MD_POFO_THEME_VERSION, true );
	}
	...
	public function hook_critical_css(): void {
		...
		$response = wp_remote_get( MD_POFO_BUILD_URI . '/inline.css' );   // load template output in buffer.

		if ( ! is_wp_error( $response ) ) {
			$css = wp_remote_retrieve_body( $response );
			wp_register_style( 'md-pofo-inline-css', false, array(), MD_POFO_THEME_VERSION, true );
			wp_add_inline_style( 'md-pofo-inline-css', $css );
			wp_enqueue_style( 'md-pofo-inline-css' );
		}
	}
	...
	public function register_styles(): void {
		...
		// Register styles.
		wp_register_style( 'main-css', MD_POFO_BUILD_URI . '/main.css', array(), MD_POFO_THEME_VERSION, 'all' );
	...
	public function register_scripts(): void {
		...
		// Register scripts.
		wp_register_script( 'main-js', MD_POFO_BUILD_URI . '/main.js', array( 'jquery-core' ), MD_POFO_THEME_VERSION, true );
	...
	public function enqueue_editor_assets(): void {
		...
		if ( is_admin() ) {
			wp_enqueue_style(
				'md-pofo-editor-css',
				MD_POFO_BUILD_URI . '/geditor.css',
				array(),
				MD_POFO_THEME_VERSION,
				'all'
			);
			wp_enqueue_script(
				'md-woodward-editor-blocks-js',
				MD_POFO_BUILD_URI . '/blocks.js',
				array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'wp-components', 'wp-data' ),
				filemtime( MD_POFO_BUILD_PATH . '/blocks.js' ),
				true
			);
		}
