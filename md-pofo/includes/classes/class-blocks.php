...
class Blocks {
	...
	protected function setup_hooks(): void {
		...
		add_filter( 'block_categories_all', array( $this, 'md_pofo_custom_block_category' ) );
	...
	public function register_theme_blocks(): void {
		...
		if ( file_exists( MD_POFO_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_POFO_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function md_pofo_custom_block_category( array $categories ): array {
		return array_merge(
			array(
				array(
					'slug'  => 'md-pofo',
					'title' => __( 'MD Pofo', 'md-pofo' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-pofo/' ) ) {
			$string = str_replace( 'md-pofo/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config(): void {
		$blocks_config = apply_filters( 'md_pofo_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'md_pofo_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
