...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_PROMOTION_MANAGER_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_PROMOTION_MANAGER_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-promotion-manager/' ) ) {
			$string = str_replace( 'md-promotion-manager/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-promotion-manager',
					'title' => __( 'MD Promotion Manager Block', 'md-promotion-manager' ),
