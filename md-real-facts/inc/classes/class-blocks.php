...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_REAL_FACTS_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_REAL_FACTS_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-real-facts/' ) ) {
			$string = str_replace( 'md-real-facts/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-real-facts',
					'title' => __( 'MD Real Facts Block', 'md-real-facts' ),
