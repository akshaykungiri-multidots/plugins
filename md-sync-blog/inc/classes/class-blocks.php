...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'md_sync_blog_custom_block_category' ) );
	}
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_SYNC_BLOG_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_SYNC_BLOG_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'md-sync-blog/' ) ) {
			$string = str_replace( 'md-sync-blog/', '', $string );
		}
	 ...
	 * @since 1.0.0
	 */
	public function md_sync_blog_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-sync-blog',
					'title' => __( 'MD Sync Blog Block', 'md-sync-blog' ),
