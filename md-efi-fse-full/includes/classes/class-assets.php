...
class Assets {
	...
	public function move_scripts_to_footer() {
		...
		wp_enqueue_script( 'jquery-core', '/wp-includes/js/jquery/jquery.min.js', array(), MD_EFI_FSE_FULL_THEME_VERSION, true );
	}
	...
	public function hook_critical_css() {
		...
		$response = wp_remote_get( MD_EFI_FSE_FULL_BUILD_URI . '/inline.css' );   // load template output in buffer.

		if ( ! is_wp_error( $response ) ) {
			$css = wp_remote_retrieve_body( $response );
			wp_register_style( 'md-efi-fse-full-inline-css', false, array(), true, true );
			wp_add_inline_style( 'md-efi-fse-full-inline-css', $css );
			wp_enqueue_style( 'md-efi-fse-full-inline-css' );
		}
	}
	...
	public function register_styles() {
		...
		wp_register_style( 'main-css', MD_EFI_FSE_FULL_BUILD_URI . '/main.css', array(), filemtime( MD_EFI_FSE_FULL_BUILD_PATH . '/main.css' ), 'all' );
	...
	public function register_scripts() {
		// Register scripts.
		wp_register_script( 'slick-min-js', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.8.1/slick.min.js', array( 'jquery' ) );
		wp_register_script( 'main-js', MD_EFI_FSE_FULL_BUILD_URI . '/main.js', array( 'jquery-core' ), filemtime( MD_EFI_FSE_FULL_BUILD_PATH . '/main.js' ), true );
	...
	public function enqueue_editor_assets() {
		...
		if ( is_admin() ) {
			wp_enqueue_style(
				'md-efi-fse-full-editor-css',
				MD_EFI_FSE_FULL_BUILD_URI . '/geditor.css',
				array(),
				filemtime( MD_EFI_FSE_FULL_BUILD_PATH . '/geditor.css' ),
				'all'
			);
		}
