<?php
/**
 * Registers the md-foursquare/md-posts block.
 *
...
/**
 *  Class for the md-foursquare/md-posts block.
 */
class MD_Posts extends Block_Base {
	...
	protected function setup_hooks() {
		add_filter( 'md_foursquare_gutenberg_blocks_config', array( $this, 'localize_block_data' ) );

		// Ajax hooks.
		add_action( 'wp_ajax_md_foursquare_load_more', array( $this, 'md_foursquare_load_more' ) );
		add_action( 'wp_ajax_nopriv_md_foursquare_load_more', array( $this, 'md_foursquare_load_more' ) );
	}
			...
			<div class="md-posts <?php echo esc_attr( $postType ); ?>">
				<div class="md-posts__filter">
					...
					foreach ( $post_taxonomies as $taxonomy => $terms ) {
						?>
						<div class="fs_select" data-texonomy="<?php echo esc_attr( $taxonomy ); ?>" data-val="">
							<span class="selected"><?php echo esc_html( get_taxonomy( $taxonomy )->label ); ?></span>
							<div class="fs_select__dropdown">
								<span class="fs_select__option active" data-val=""><?php echo esc_html( get_taxonomy( $taxonomy )->label ); ?></span>
								<?php
								foreach ( $terms as $term ) {
									?>
									<span class="fs_select__option" data-val="<?php echo esc_attr( $term->term_id ); ?>"><?php echo esc_html( $term->name ); ?></span>
									<?php
								}
								?>
							</div>
				...
				<?php if ( $query->have_posts() ) : ?>
					...
					<?php if ( $query->max_num_pages > 1 ) : ?>
						<div class="md-posts__loadmore">
							<button class="md-posts__loadmore-button btn-main" data-page="1" data-post-type="<?php echo esc_attr( $postType ); ?>" data-posts-per-page="<?php echo esc_attr( $postsToShow ); ?>" data-post-in-row="<?php echo esc_attr( $postInRow ); ?>">
								<?php esc_html_e( 'View More', 'md-foursquare' ); ?>
							</button>
						</div>
					<?php endif; ?>
				<?php else : ?>
					<p><?php esc_html_e( 'No posts found.', 'md-foursquare' ); ?></p>
				<?php endif; ?>
			</div>
		</div>
		<?php
	 ...
	 * @since 1.0.0
	 */
	public function md_foursquare_load_more() {
		// Check nonce.
