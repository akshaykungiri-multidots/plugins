...
class Blocks {
	...
	public function register_blocks() {
		...
		if ( file_exists( MD_WALLET_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( MD_WALLET_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		$namespace = 'MD_Wallet\Blocks\\';

		// Remove namespace from block name.
		if ( false !== strpos( $string, 'md-wallet/' ) ) {
			$string = str_replace( 'md-wallet/', '', $string );
		}
	...
	public function mdprime_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'md-wallet',
					'title' => __( 'MD Wallet Block', 'md-wallet' ),
